...
const initialData = {
  users: [],
  topic_progress: [],
  subtopic_progress: [], // Array of { id, user_id, subtopic_key, completed, updated_at }
  custom_holidays: [],
  uploaded_materials: [],
  lastIds: {
    users: 0,
...
function loadDb() {
  ...
  try {
    ...
    if (!data.subtopic_progress) data.subtopic_progress = [];
    if (!data.lastIds) {
      data.lastIds = {
        ...
        subtopic_progress: data.subtopic_progress.length ? Math.max(...data.subtopic_progress.map(s => s.id)) : 0,
...
const db = {
  ...
  // Subtopic Progress (Checkboxes persistence)
  getSubtopicProgressByUser: (userId) => {
    const data = loadDb();
    return data.subtopic_progress.filter(sp => sp.user_id === userId);
  },
  upsertSubtopicProgress: (userId, subtopicKey, completed) => {
    const data = loadDb();
    let record = data.subtopic_progress.find(sp => sp.user_id === userId && sp.subtopic_key === subtopicKey);
    if (record) {
      record.completed = completed ? 1 : 0;
      record.updated_at = new Date().toISOString();
    } else {
      data.lastIds.subtopic_progress += 1;
      record = {
        id: data.lastIds.subtopic_progress,
        user_id: userId,
        subtopic_key: subtopicKey,
        completed: completed ? 1 : 0,
        updated_at: new Date().toISOString()
      };
      data.subtopic_progress.push(record);
    }
    saveDb(data);
    return record;
  },
