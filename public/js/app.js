...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let subtopicProgress = {}; // { subtopic_key: boolean }
  let customHolidays = []; // [{ id, holiday_date, title }]
  let uploadedMaterials = [];

  // Default date automatically opens to current date (today)
  let currentDateObj = new Date();
  let sideCalendarMonthDate = new Date();
  ...
  async function syncUserProgress() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        subtopicProgress = data.subtopics || {};
  ...
  function setupEventListeners() {
    ...
    document.getElementById('sidebar-excel-btn')?.addEventListener('click', downloadExcelReport);
  ...
  function renderStudyDetails() {
    ...
    container.innerHTML = `
      <div class="study-card ${isSunday || OFFICIAL_HOLIDAYS_2026[formattedDate] ? 'holiday-card' : ''}">
        ...
        <div class="study-card-body">
          ...
          <!-- Subtopics Checklist (with database persistence) -->
          <div class="subtopics-box">
            ...
            <div class="subtopics-list">
              ${scheduleItem.subtopics.map((st, idx) => {
                const subtopicKey = `${formattedDate}_subtopic_${idx}`;
                const checked = subtopicProgress[subtopicKey] ? 'checked' : '';
                return `
                  <label class="subtopic-item">
                    <input type="checkbox" class="subtopic-checkbox" data-key="${subtopicKey}" ${checked}>
                    <span>${st}</span>
                  </label>
                `;
              }).join('')}
    ...
    // Subtopics checkbox change event
    container.querySelectorAll('.subtopic-checkbox').forEach(chk => {
      chk.addEventListener('change', async (e) => {
        const subtopicKey = chk.dataset.key;
        const isChecked = chk.checked;
        subtopicProgress[subtopicKey] = isChecked;

        if (jwtToken) {
          try {
            await fetch('/api/progress/subtopic', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${jwtToken}`
              },
              body: JSON.stringify({ subtopicKey, completed: isChecked })
            });
          } catch (err) {
  ...
  // --- EXCEL (.CSV) PROGRESS REPORT EXPORT ---
  function downloadExcelReport() {
    let csvRows = [];
    csvRows.push(["Date", "Day", "Subject", "Topic Title", "Subtopic", "Subtopic Status", "Overall Day Status"]);

    DAILY_SCHEDULE_RAW.forEach(s => {
      const dayDate = new Date(s.date);
      const dayOfWeek = dayDate.toLocaleDateString('en-US', { weekday: 'short' });
      const dayStatus = userProgress[s.date]?.completed ? "Completed" : "Pending";

      s.subtopics.forEach((st, idx) => {
        const subtopicKey = `${s.date}_subtopic_${idx}`;
        const subtopicStatus = subtopicProgress[subtopicKey] ? "Completed" : "Pending";
        csvRows.push([
          s.date,
          dayOfWeek,
          `"${s.subject}"`,
    ...
    link.href = url;
    link.download = `GATE_CS_2027_Preparation_Report_${formatDateYMD(new Date())}.csv`;
    link.click();
