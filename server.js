...
app.get('/api/progress', authenticateToken, (req, res) => {
  try {
    ...
    const subtopicRows = db.getSubtopicProgressByUser(req.user.userId);
    const subtopicsMap = {};
    subtopicRows.forEach(s => {
      subtopicsMap[s.subtopic_key] = s.completed === 1;
    });

    res.json({ topics: progressMap, subtopics: subtopicsMap });
  } catch (err) {
...
app.post('/api/progress/subtopic', authenticateToken, (req, res) => {
  const { subtopicKey, completed } = req.body;
  if (!subtopicKey) {
    return res.status(400).json({ error: 'subtopicKey is required.' });
  }

  try {
    const record = db.upsertSubtopicProgress(req.user.userId, subtopicKey, completed);
    res.json({ subtopicKey: record.subtopic_key, completed: record.completed === 1 });
  } catch (err) {
    console.error('Error toggling subtopic progress:', err);
    res.status(500).json({ error: 'Failed to update subtopic progress.' });
  }
});
